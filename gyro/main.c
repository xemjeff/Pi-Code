...
#define DT 0.09         // [s/loop] loop period. 20ms
...
int main(int argc, char *argv[])
{
	...
	//      Complementary filter used to combine the accelerometer and gyro values.
		...
		mvprintw(2,16,"X: %7.3f\n",AccXangle);
		mvprintw(3,16,"Y: %7.3f\n",AccYangle);
		...
		//Our loop takes 60-70 ms, and we want them all the same.
	    while(mymillis() - startInt < 90)
	    {
	        usleep(100);
	    }
		mvprintw(6,0,"Loop Time %6.1f", mymillis());
		refresh();	    
